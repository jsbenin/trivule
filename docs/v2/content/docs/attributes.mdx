...
6. `@v:submit`, `@v:enabled-class`, and `@v:disabled-class`:
...
7. `@v:label`: 
   ...
   - Example: `<input type="text" @v:label="our conditions">`
...
9. `@v:debounce`:
   - Purpose: This attribute allows you to set a delay (in milliseconds) before the validation is executed after an event is triggered.
   - Use Case: Highly useful for performance optimization and **async validation** (e.g., checking if an email is already taken) to avoid sending too many requests to the server while the user is still typing.
   - Example: `<input type="text" @v:rules="required|email" @v:debounce="500">`

Note: Remember to include the Trivule library in your application before using these attributes. You can find the library and code examples on the [Trivule GitHub repository](https://github.com/jsbenin/trivule).
