...
If you have a website or application and want to validate all forms without managing each form individually, you can use global validation with [declarative validation](/docs/validation-modes#declarative-validation).
...
This will activate the validation for all your forms. Trivule will validate any set of HTML fields with @v:rules in , whether inside a `<form>` .
...
```html
<form @v:form>
  <input type="text" name="username" @v:rules="required|minlength:5" />
  <input type="email" name="email" @v:rules="required|email" />
  <input type="password" name="password" @v:rules="required|minlength:8" />
  <button type="submit" @v:submit>Submit</button>
</form>
```
...
[Imperative validation](/docs/validation-modes#imperative-validation) allows you to define validation rules and apply them dynamically using JavaScript code. This offers greater flexibility for more complex validation cases.
...
-  `feedbackSelector` (optional): If this option is not specified, Trivule will first check if a selector is indicated during declaration with the `make` method, and then use that selector. In the absence of a selector, Trivule will check if the input has an associated HTML element with the `@v:feedback` attribute set to the input's name. If no element is found in either of these cases, no feedback message will be displayed.
...
const options = {
  ...
  feedbackSelector: "[@v:feedback={name}]", //where {name} we be replaced with the input name
  realTime: true;
