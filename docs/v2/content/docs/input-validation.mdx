...
- **`feedbackElement` (optional):** Specifies where validation error messages are displayed, either through a CSS selector or by checking for an associated HTML element with the `@v:feedback` attribute set to the input's name.
...
- **`debounce` (optional):** Time in milliseconds to delay validation after an event. Especially useful for **async validation** (explained below).
...
The `debounce` option (or `@v:debounce` attribute) is crucial when dealing with real-time validation and **async rules**. 

Without debouncing, validation triggers on every keystroke. If your rule performs an expensive operation or an API call (like checking username availability), this can lead to performance issues or server overloading.

```html
<!-- Validate 500ms after the user stops typing -->
<input 
  name="username" 
  @v:rules="required|min:3|check_unique" 
  @v:debounce="500" 
/>
```

By adding a debounce, you ensure that the validation logic only runs after the user has paused typing for the specified duration.
...
trivuleInput.insertAfterRule("min_length",{ 
  rule: "customRule",
  validate: (value) => {
    return {
        passes: value === "customValue",
...
trivuleInput.insertBeforeRule("max_length",{ 
  rule: "customRule",
  validate: (value) => {
    return {
        passes: value === "customValue",
